...
const BobRossGallery: React.FC = () => {
  ...
  // 1. Change state to an array for multiple selections
  ...
  const { seasons, absoluteMax, masterPalette } = useMemo(() => {
    ...
    return {
      ...
      masterPalette: Array.from(colorMap.entries()).map(([name, hex]) => ({ name, hex }))
    };
  }, [paintings]);

  // 2. Logic to toggle colors in and out of the selection array
  const toggleColor = (colorName: string) => {
    setSelectedColors(prev => 
      prev.includes(colorName) 
        ? prev.filter(c => c !== colorName) // Remove if already there
        : [...prev, colorName]               // Add if not there
    );
  };

  const filteredPaintings = useMemo(() => {
    return paintings.filter((painting) => {
      const matchesSearch = painting.painting_title.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesSeason = selectedSeason === 'All' || painting.season === Number(selectedSeason);
      const matchesRange = painting.num_colors >= minColors && painting.num_colors <= maxColorsFilter;
      
      // 3. Multi-select logic: Does the painting contain ALL selected colors?
  ...
  }, [searchTerm, selectedSeason, minColors, maxColorsFilter, selectedColors, paintings]);
  return (
    <div className="gallery-container">
      {selectedId === null ? (
        <>
          <header className="gallery-header">
            ...
            <div className="controls">
              <div className="main-filters">
                ...
                >
                  {seasons.map(s => (
                    <option key={s} value={s}>{s === 'All' ? 'All Seasons' : `Season ${s}`}</option>
                  ))}
                </select>
              </div>

              <div className="master-palette-container">
                <label>Filter by Paint Colors ({selectedColors.length}):</label>
                <div className="master-palette">
                  {masterPalette.map((color) => (
                    <button
                      key={color.name}
                      className={`palette-swatch ${selectedColors.includes(color.name) ? 'active' : ''}`}
                  ...
                  {selectedColors.length > 0 && (
                    <button className="clear-color" onClick={() => setSelectedColors([])}>
                      Reset Colors
                    </button>
                  )}
                </div>
              ...
              <div className="color-controls-column">
                <div className="range-group">
                  ...
                  <input 
                    type="range" min="0" max={absoluteMax} value={minColors} 
                ...
                <div className="range-group">
                  ...
                  <input 
                    type="range" min="0" max={absoluteMax} value={maxColorsFilter} 
          ...
          <PaintingList paintings={filteredPaintings} onSelectPainting={setSelectedId} />
        </>
      ) : (
        <PaintingDetail paintingId={selectedId} onBack={() => setSelectedId(null)} />
      )}
    </div>
  );
};
